...
class image_projection:
    ...
    def image_color_callback(self, data):
        ...
        for c in hsv_contours:
            ...
            perm = permutations([1, -1, 0, -2, 2], 2)
            next_pixel = [0,-1, -2, 1, 2]

            if (numpy.isnan(depth_value)):
                for i in next_pixel:
                    for j in next_pixel: 
                        new_depth_coords_x = int(depth_coords[0]) + i
                        new_depth_coords_y = int(depth_coords[1]) + j
                        depth_value = image_depth[int(new_depth_coords_x), int(new_depth_coords_y)]
                        if numpy.isnan(depth_value):
                            continue
                        else:
                            depth_coords[0] = new_depth_coords_x
                            depth_coords[1] = new_depth_coords_y
                            break 
            ...
            xyz.append(depth_value)
            xyz.append("depth cords")
            xyz.append(int(depth_coords[0]))
            xyz.append(int(depth_coords[1]))
            xyz.append("image cords")
            xyz.append(image_coords[0])
            xyz.append(image_coords[1])
        ...
        global way_point, image_count
        way_point = "Take_Image"      
